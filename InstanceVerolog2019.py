...
def passInstance(instanceTxt,debug):
    instance=instanceTxt
    Instance = InstanceVerolog2019(instance,None,False)
    if debug is True:
        if Instance.isValid():
            print('Instance %s is a valid Verolog2019 instance' % instance)
            if not False:
                res = Instance.areDistancesValid()
                print(res[1])
            if False:
                Instance.writeInstance(None,False)
