...
class SolutionVerolog2019(base.BaseParser):
    ...
    class LANG:
        class TXT:
            ...
            idleMachineCost ='IDLE_MACHINE_COST'
    ...
    def _readDay(self, fd, lastLineAssignment):
        ...
        if newDay.NrTechnicianRoutes > 0:
          while lastLineAssignment is not None and lastLineAssignment[0] is None:
              line = lastLineAssignment[1]
              routeLine = line.split()
    ...
    def areGivenValuesValid(self):
        try:
            self._checkError('Incorrect truck distance (%d instead of %d).' % (self.givenCost.TruckDistance if self.givenCost.TruckDistance is not None else 0, self.calcCost.TruckDistance), self.givenCost.TruckDistance is None or self.givenCost.TruckDistance == self.calcCost.TruckDistance )
            self._checkError('Incorrect number of truck days (%d instead of %d).' % (self.givenCost.NrTruckDays if self.givenCost.NrTruckDays is not None else 0,self.calcCost.NrTruckDays), self.givenCost.NrTruckDays is None or self.givenCost.NrTruckDays == self.calcCost.NrTruckDays )
            self._checkError('Incorrect number of trucks used (%d instead of %d).' % (self.givenCost.NrTrucksUsed if self.givenCost.NrTrucksUsed is not None else 0,self.calcCost.NrTrucksUsed), self.givenCost.NrTrucksUsed is None or self.givenCost.NrTrucksUsed == self.calcCost.NrTrucksUsed )
            self._checkError('Incorrect technician distance (%d instead of %d).' % (self.givenCost.TechnicianDistance if self.givenCost.TechnicianDistance is not None else 0,self.calcCost.TechnicianDistance), self.givenCost.TechnicianDistance is None or self.givenCost.TechnicianDistance == self.calcCost.TechnicianDistance )
            self._checkError('Incorrect number of technician days (%d instead of %d).' % (self.givenCost.NrTechnicianDays if self.givenCost.NrTechnicianDays is not None else 0,self.calcCost.NrTechnicianDays), self.givenCost.NrTechnicianDays is None or self.givenCost.NrTechnicianDays == self.calcCost.NrTechnicianDays )
            self._checkError('Incorrect number of technician used (%d instead of %d).' % (self.givenCost.NrTechniciansUsed if self.givenCost.NrTechniciansUsed is not None else 0,self.calcCost.NrTechniciansUsed), self.givenCost.NrTechniciansUsed is None or self.givenCost.NrTechniciansUsed == self.calcCost.NrTechniciansUsed )
            self._checkError('Incorrect idle machine cost (%d instead of %d).' % (self.givenCost.IdleMachineCost if self.givenCost.IdleMachineCost is not None else 0,self.calcCost.IdleMachineCost), self.givenCost.IdleMachineCost is None or self.givenCost.IdleMachineCost == self.calcCost.IdleMachineCost )
            self._checkError('Incorrect total cost (%d instead of %d).' % (self.givenCost.Cost if self.givenCost.Cost is not None else 0,self.calcCost.Cost), self.givenCost.Cost is None or self.givenCost.Cost == self.calcCost.Cost )
